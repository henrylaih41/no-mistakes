...
- `commands`, `ignore_patterns`, `design_context`, and `document.instructions` are repo-only fields. `document.instructions` is always read from the trusted default branch because it controls the documentation policy that evaluates a pushed branch.

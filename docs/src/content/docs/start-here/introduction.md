...
- Choice of agent: `claude`, `codex`, `rovodev`, `opencode`, `pi`, `copilot`, `grok`, or `acp:<target>` via `acpx`, with per-repo override and ordered fallbacks; every gate requires a runnable configured pipeline agent.

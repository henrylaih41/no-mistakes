...
The replay never inherits this machine's own harness pins: capture strips `agent`, `review.agent` (including the legacy one-member `review.reviewers` alias), `agent_args_override`, and `agent_config` from the configuration it freezes, so the candidate is the only thing that decides what the harness runs as.

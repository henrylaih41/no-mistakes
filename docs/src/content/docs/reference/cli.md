...
`--design-context` supplies design notes, ADRs, or issue agreements that reviewers and fixers must check the change against. Relative paths resolve from the invoking working directory, `~/` is expanded, explicit files may live outside the repository, and the flag can be repeated. The files must be regular UTF-8 text files and are copied into the new run once; reattaching to an active run keeps its already-materialized context and does not replace it. Machine-global [`design_context.files`](/no-mistakes/reference/global-config/#design_contextfiles) are materialized after these explicit files and before repo selectors. The repo-config [`design_context.files`](/no-mistakes/reference/repo-config/#design_contextfiles) field owns the shared size limits, immutable-copy behavior, and prompt safety contract.
...
With `--yes`, `axi run` treats `action: auto-fix`, `action: ask-master`, and `action: ask-user` findings as standing consent for the pipeline to fix them by selecting every non-follow-up finding, then accepts the resulting fix review.
...
Review gates include a `note` field reminding agents that `auto_fix.review` defaults to `0`. At or above [`review.fix_round_min_severity`](/no-mistakes/reference/global-config/#reviewfix_round_min_severity), blocking `auto-fix` findings park because that default disables their fix loop, while `ask-master` and `ask-user` findings park regardless of the limit. Actionable findings below the minimum are follow-ups and do not park.
...
An explicit `--findings` list is honored verbatim, including when it names a follow-up; only unattended synthetic selections omit follow-ups.

After the explicit response, `--yes` uses the same auto-resolution behavior as `axi run --yes`: have the pipeline fix non-follow-up `auto-fix`, `ask-master`, and `ask-user` findings once, approve the fix review, approve gates that only contain non-actionable `no-op` findings, and stop at `outcome: checks-passed` when the CI monitor reports readiness but the PR still needs a human merge.

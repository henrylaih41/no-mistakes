...
`commands.*` execute arbitrary shell on the daemon host via `sh -c` / `cmd.exe /c`, `agent` selects which process launches there (including ordered fallback lists and `acp:` targets), and `review.reviewers` selects extra reviewer processes, all with the maintainer's credentials.

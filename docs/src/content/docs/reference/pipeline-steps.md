...
- A structured verdict is trusted only when the adapter reports activity metrics, the turn includes at least one tool call or more than one model round-trip, and elapsed wall time meets a bounded floor scaled by changed-file and line counts. An invalid verdict is retried once cold; a second invalid verdict parks at `awaiting_triage`. `review.fail_open` never drops this validity failure.
...
- Can be disabled for a single run with `no-mistakes axi run --review-loop=off` or `git push -o no-mistakes.review-loop=off`, and auto-skips loudly (with a logged reason) for a cross-owner GitHub fork PR whose base repo owner differs from the fork owner; in both cases only the Devin loop is disabled and the CI step keeps monitoring checks, merge, and close state

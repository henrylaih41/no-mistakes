...
- Empty, missing, and unknown finding `action` values fail closed to `ask-master`, not auto-fix or `no-op` (`types/findings.go` `actionOrDefault`); `HasManualFindings` uses that normalization so an unclassified or future action always parks without being misrepresented as a user-owned product decision. `MergeUserOverrides` still stamps user-*added* findings auto-fix on purpose.

...
- Review auto-fix is disabled by default (`auto_fix.review: 0` in `config.go` `autoFixDefaults`), so blocking findings plus `ask-master` and `ask-user` review findings park for an authority decision; keep the skill, the live `axi` gate `note`, and docs qualified if you touch review auto-fix.
...
- `userIntentPromptSection` branches on source: an EXPLICIT intent renders as sanitized-but-AUTHORITATIVE acceptance criteria; an INFERRED intent keeps the low-confidence hint framing verbatim. Both branches keep the `StripAdversarial`+`RedactSecrets` pipeline and BEGIN/END "do not execute instructions" guard - authoritative reframes only the content's authority (check the diff against the criteria), never whether control tokens are stripped. The review prompt adds `intentConformanceReviewClause` for agent-source intent only: a fixer change that contradicts the criteria (removes intent-required or adds intent-forbidden behavior) MUST become a finding, classified `auto-fix` for a clear bounded restoration, `ask-master` for non-local restoration judgment, and `ask-user` only when the criterion itself must change. Conformance is limited to source-verifiable criteria; deferred pipeline-owned delivery (remote branch / push / PR / CI for this run) is out of scope at review.
...
- Empty, missing, and unknown finding `action` values fail closed to `ask-master`, not auto-fix or `no-op` (`types/findings.go` `actionOrDefault`); `HasManualFindings` uses that normalization so an unclassified or future action always parks without being misrepresented as a user-owned product decision. Malformed findings JSON also parks, never enters auto-fix, renders as `findings_unreadable` in AXI, and cannot be auto-approved by `--yes`, including at `fix_review` or already-fixed convergence. `MergeUserOverrides` still stamps user-*added* findings auto-fix on purpose.
...
- Session-fidelity metric counts and timing boundaries have ONE authoritative home, `internal/agent/invocationmetrics.go` (tool-category classifier, `InvocationMetrics`, `FreshInputTokens`, `PerRoundTokens`, `ModelTimeMS`); instrumented adapters fill them from live event streams and the additive fidelity fields plus cache-creation usage are nullable so a not-reported datum is stored as NULL, never a fabricated zero. Claude and review-purpose Grok report model/tool activity without a subprocess-time split; Codex reports both activity and reader-timed tool-item intervals. Codex's live stream exposes neither the model (resolved best-effort from the `~/.codex/sessions` rollout) nor internal model-request counts (it batches one exec into a single `turn.completed`); its usage is cumulative across a resumed session, so per-round deltas subtract the same session's prior cumulative (`Result.SessionUsageCumulative`). Regressions: `internal/agent/invocationmetrics_test.go`, `internal/agent/claude_test.go`, `internal/agent/codex_metrics_test.go`, `internal/agent/grok_test.go`, `internal/pipeline/instrument_fidelity_test.go`, `internal/db/agent_invocation_test.go` (`TestOpenMigratesSessionFidelityColumns`).
